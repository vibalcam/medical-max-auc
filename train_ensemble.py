...
def main(args):
    ...
    models = [GeneralModel(args=args, img_shape=train_dataset[0][0].shape, num_outputs=n_outputs) for _ in range(args.num_models)]
    # model_task = Module(
    #     args=args,
...
# #         self.models = nn.ModuleList([Module(args=args, img_shape=img_shape, num_outputs=n_outputs) for _ in range(n_models)])
...
# # Define the ensemble model which consists of multiple ResNet18 models
...
#         # args.pretrained = os.path.join(logger_base, args.pretrained)
#         print(f"==>Loading pretrained model from {args.pretrained}")
#         model = EnsembleModule.load_from_checkpoint(args.pretrained).model
